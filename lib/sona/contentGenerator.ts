/**
 * SONA v4 Content Generator - Enhanced Version
 * مولد المحتوى المحسّن - يولد مقالات عربية غنية ومخصصة
 */
...
export class ContentGenerator {
  ...
  async generate(request: GenerationRequest): Promise<GeneratedContent> {
    ...
    if (request.category) analysis.category = request.category;
    ...
    const qualityReport = this.calculateQuality(content, keywords);
