...
async function searchPexels(query: string, perPage: number = 15, page: number = 1): Promise<PexelsImage[]> {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!query) {
      return NextResponse.json({ error: 'Query parameter is required', images: [] }, { status: 400 });
    }

    console.log(`[Images Search API] Query: "${query}", Count: ${count}`);
    const englishQuery = topicToEnglishKeywords(query) || 'celebration';
    console.log(`[Images Search API] English query: "${englishQuery}"`);

    const allImages: UnifiedImage[] = [];
    const seenIds = new Set<string>();

    const pexelsImages = await searchPexels(englishQuery, Math.ceil(count * 0.7), page);
    ...
    const unsplashImages = await searchUnsplashImages(englishQuery, Math.ceil(count * 0.5), page);
