...
// Check بيك سيكس database
const bekSixPath =
  'C:\\web\\secend_stadge\\miladak_v2\\بيك سيكس\\database.sqlite';

console.log('Checking بيك سيكس database...\n');
console.log(`Path: ${bekSixPath}`);

try {
  if (!fs.existsSync(bekSixPath)) {
    console.log('❌ File does not exist');
    return;
  }

  const db = new Database(bekSixPath);
  ...
  if (hasPageKeywords) {
    ...
    if (count.count > 5) {
      ...
      console.log(`\n🔥 FOUND THE FULL BACKUP: ${bekSixPath}\n`);
    }
  } else {
