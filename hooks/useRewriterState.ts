...
export function useRewriterState() {
  ...
  // ===== Smart Formatting =====
  const smartFormatContent = useCallback(async () => {
    if (!state.rewrittenContent.trim()) {
      setError('لا يوجد محتوى لتنسيقه');
      return;
    }

    setState((prev) => ({ ...prev, isSmartFormatting: true }));
    setError(null);

    try {
      // استيراد المنسق الذكي
      const { smartFormatContent } = await import('@/lib/utils/smartFormatter');

      // تطبيق التنسيق الذكي
      const formattedContent = smartFormatContent(state.rewrittenContent);

      // تحديث المحتوى المنسق
      setRewrittenContent(formattedContent);
      setSuccess('تم تطبيق التنسيق الذكي بنجاح');
    } catch (err: any) {
      console.error('Error in smart formatting:', err);
      setError('حدث خطأ أثناء التنسيق الذكي');
    } finally {
      setState((prev) => ({ ...prev, isSmartFormatting: false }));
    }
  }, [state.rewrittenContent, setRewrittenContent]);
  ...
  const canSmartFormat = useMemo(() => {
    return state.rewrittenContent.trim().length > 0 && !state.isSmartFormatting;
  }, [state.rewrittenContent, state.isSmartFormatting]);

  const isProcessing = useMemo(() => {
    return state.isLoading || state.isFetching || state.isSmartFormatting;
  }, [state.isLoading, state.isFetching, state.isSmartFormatting]);
